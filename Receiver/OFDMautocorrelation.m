% Autocorrelation of a signal that is not strictly following definition of autocorrelation in MATLAB.
% ALGORITHM:
...
% 
% INPUTS:
%     signal - signal for autocorrelation (array of complex numbers)
%     Nd - size of useful data - size of symbol without guard interval (integer)
%     window_size - size of a window in which autocorrelation is performed - described in algorithm above
% 
% OUTPUTS:
%     autocorrelation - each element represents one sample of autocorrelation (array of doubles)
...
ofdm_autocorr = zeros(1, n+Nd-win+1);
for i=1:n+Nd-win+1
    ofdm_autocorr(i) = abs(block(i:i+win-1) * block_lagged(i:i+win-1)');                                        %when complex array is transposed, it also gets
end                                                                                                             %conjugated
